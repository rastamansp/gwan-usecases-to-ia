...
// When - Ações
When('eu envio uma requisição POST para {string} na API de busca', async function(this: CustomWorld, endpoint: string) {
...
When('eu envio uma requisição GET para {string} na API de busca', async function(this: CustomWorld, endpoint: string) {
  const response = await BDDTestUtils.executeRequest(
    this,
    'GET',
    endpoint
...
// Then - Validações
Then('a resposta da API de busca deve ter status {int}', async function(this: CustomWorld, expectedStatus: number) {
...
Then('o status da busca na API deve ser {string}', async function(this: CustomWorld, expectedStatus: string) {
  const response = this.getResponse();
  const data = response.body;
