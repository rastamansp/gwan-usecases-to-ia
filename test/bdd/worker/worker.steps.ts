...
Given('que tenho acesso à API de gerenciamento do worker', async function(this: CustomWorld) {
  ...
  const response = await BDDTestUtils.executeRequest(
    this,
    'GET',
    '/api/worker/health'
  );
...
// When - Ações
When('eu envio uma requisição GET para {string} na API do worker', async function(this: CustomWorld, endpoint: string) {
  const response = await BDDTestUtils.executeRequest(
    this,
    'GET',
    endpoint
...
When('eu envio uma requisição POST para {string} na API do worker', async function(this: CustomWorld, endpoint: string) {
  const response = await BDDTestUtils.executeRequest(
    this,
    'POST',
    endpoint
...
// Then - Validações
Then('a resposta da API do worker deve ter status {int}', async function(this: CustomWorld, expectedStatus: number) {
  const response = this.getResponse();
  expect(response.status).to.equal(expectedStatus);
});

Then('a API do worker deve retornar status {string}', async function(this: CustomWorld, expectedStatus: string) {
  const response = this.getResponse();
...
Then('o status da busca no worker deve ser {string}', async function(this: CustomWorld, expectedStatus: string) {
  const response = this.getResponse();
  const data = response.body;
